...
function [err_SVM, cm_SVM, svmTestRunningTime, prediction_SVM_Test] = Classify_LIBSVM_Test(trainData, ...
                                 trainLabels, ...
                                 testData, ...
                                 testLabels, ...
                                 nClasses,... 
...
for i = 1:numel(g)       
    for j = 1:numel(C)       
        for classNumber = 1:nClasses
            ...
            % Train the model
            svmLearningModel = svmtrain(testOneVsAllLabels, trainData, svmLearningParams);
            ...
            tic
            % Test the model
            [svmPredictedLabels, ~, ~] = svmpredict(testLabels, testData, svmLearningModel);                
...
err_SVM = error_mtx/(length(testLabels));
cm_SVM=confusionmat(testLabels,predicted); 
svmTestRunningTimeAll = toc;
svmTestRunningTime{3, 1} = svmTestRunningTimeAll;
save libsvmTestResults
end                            
